...
# Add minS condition, eliminating races (sCats) with less than 50 (sLim) ppl
allSCats <- unique(data[,sName])
sCats <- factor()
for (i in 1:length(allSCats)) {
...
ggplot() + 
geom_smooth(data=plotdf1, mapping=aes(x=cats, y=preds, color='Caucasian'),se=F) + 
geom_smooth(data=plotdf2, mapping=aes(x=cats, y=preds, color='African-American'),se=F) + 
geom_smooth(data=plotdf3, mapping=aes(x=cats, y=preds, color='Hispanic'),se=F) + 
labs(x='age', y='two_year_recid',color='Legend') + 
...
# Build color list for legend
buildColors <- c("colors = c(")
for (i in 1:(length(sCats)-1)) {
  addColor <- paste("'",as.character(sCats[i]),"' = '",colors[i],"',",sep="")
  buildColors <- paste(buildColors,addColor,sep="")
}
addColor <- paste("'",as.character(sCats[i]),"' = '",colors[i],"')",sep="")
buildColors <- paste(buildColors,addColor,sep="")
